...
We start by using the hint and introduce an arbitrary test funciton $g(x)$. If we write
\begin{equation}
\int_{-\infty}^{+\infty} \delta[f(x)]g(x)dx
\end{equation}
Then, for every $t_i$, split the integral into parts, namely
\begin{equation}
\int_{-\infty}^{+x_0-\epsilon} \delta[f(x)]g(x)dx + \int_{-x_0-\epsilon}^{+x_0 + \epsilon} \delta[f(x)]g(x)dx + \int_{x_0 + \epsilon}^{+\infty} \delta[f(x)]g(x)dx
\end{equation}
